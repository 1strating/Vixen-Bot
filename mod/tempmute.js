...
console.log(e.stack)
  ...
  let muteTime = args[2];
  if(!muteTime) return message.channel.send("You must specify the time limit of this users mute")
  
  await (member.roles.add(muteRole.id))
  message.channel.send(`**${member.user.tag}** has been muted for \`${ms(ms(muteTime))}\``)
